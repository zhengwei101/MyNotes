...
# print(mixList.extend(numlist))  # extend的返回值是None
# print(mixList)
...
print("sort before ", numlist1)
numlist.sort()
print("sort after  ",numlist)

# 这样，对numList的操作就不会影响到numList1了。

# 访问list中的元素
print("numlist[0]    ", numlist[0])  
print("numlist[0:3]  ", numlist[0:3])
print("numlist[0:-2] ", numlist[0:-2])

numlist.insert(1,999) # insert的第一个参数是要插入的位置，第二个参数是要插入的值。
print("after insert ", numlist)

numlist.insert(100,2) # 如果你使用的位置超过了list的最大长度，insert就会把元素插在list末尾
print("after insert ",numlist)
...
del(numlist[0:5:2]) # 表示从位置0开始，每两个元素删掉一个，一直到位置5。因此，执行之后，结果就是[6, 8]了。
